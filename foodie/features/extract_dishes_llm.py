...
── Usage ────────────────────────────────────────────────────────────────────────
    # Ollama (default)
    python -m foodie.features.extract_dishes_llm
    python -m foodie.features.extract_dishes_llm --url http://localhost:11434 \
        --model-name hf.co/unsloth/Qwen3.5-27B-GGUF:Q4_K_M

    # SGLang / vLLM
    python -m foodie.features.extract_dishes_llm --url http://localhost:8000 --model-name default

    python -m foodie.features.extract_dishes_llm --reviews-per-call 15 --concurrency 20
    python -m foodie.features.extract_dishes_llm --merge-meta           # keep meta dishes
    python -m foodie.features.extract_dishes_llm --resume               # resume
    python -m foodie.features.extract_dishes_llm --health-only          # health check
...
async def run_extraction(
    ...
    if not REVIEWS_FLAT.exists():
        log.error(
            f"Missing {REVIEWS_FLAT} — run: "
            "python -m foodie.features.build_graph_data --step 1b"
        )
        return
