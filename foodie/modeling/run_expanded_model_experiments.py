...
MODEL_SPECS = {
    "two_tower": ("foodie.modeling.recommendation_two_tower", ["--batch-size", "32768"]),
    "lightgcn": ("foodie.modeling.recommendation_gnn", ["--layers", "3", "--batch-size", "4096"]),
    "kgat_sal": ("foodie.modeling.recommendation_kgat_sal",
...
def prepare_features() -> None:
    ...
    cmd = [PYTHON, "-m", "foodie.features.build_training_features",
           "--llm-feat-mode", "full",
...
def run_models(runs: list[dict]) -> None:
    ...
    while queue or active:
        for gpu in (0, 1):
            ...
            cmd = [PYTHON, "-m", run["script"], *run["args"]]
...
def main() -> None:
    ...
    runs = make_runs()
    run_models(runs)
    subprocess.run([PYTHON, "-m", "foodie.modeling.summarize_expanded_experiments"], cwd=ROOT,
